...
__all__ = [
    ...
    # Everything below is private (leading underscore) or a tuning constant, not
    # part of the intended public API -- it is listed here only because the
    # module docstring above promises every name from the former monolithic
    # ``inpainting_engines.py`` stays importable as ``pdf2ppt.inpainting_engines.<name>``,
    # including as an ``unittest.mock.patch`` target. Listing them in ``__all__``
    # documents that re-export contract explicitly (ruff's own suggested fix for
    # "imported but unused" here) instead of suppressing the check per line.
    "DEFAULT_LAMA_COMPOSITE_ALPHA_GAIN",
    "DEFAULT_LAMA_COMPOSITE_BLEND_SIGMA",
    "DEFAULT_LAMA_COMPOSITE_HARD_CORE_ERODE_PX",
    "DEFAULT_LAMA_COMPOSITE_MASK_DILATE_PX",
